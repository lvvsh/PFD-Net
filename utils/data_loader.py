...
def randomCrop_Mosaic_Semi(image_A, image_B, label, image_A_diff, image_B_sharp, crop_win_width, crop_win_height):
        image_width = image_A.size[0]
        image_height = image_A.size[1]
        random_region = (
        (image_width - crop_win_width) >> 1, (image_height - crop_win_height) >> 1, (image_width + crop_win_width) >> 1,
        (image_height + crop_win_height) >> 1)
        return image_A.crop(random_region), image_B.crop(random_region), label.crop(random_region), image_A_diff.crop(random_region), image_B_sharp.crop(random_region)

def cv_random_flip(img_A, img_B, label, img_A_diff, img_B_sharp):
    # left right flip
    ...
    if flip_flag == 1:
        ...
        img_A_diff = img_A_diff.transpose(Image.FLIP_LEFT_RIGHT)
        img_B_sharp = img_B_sharp.transpose(Image.FLIP_LEFT_RIGHT)
    return img_A, img_B, label, img_A_diff, img_B_sharp
...
def randomCrop(image_A, image_B, label, image_A_diff, image_B_sharp):
    border = 30
    image_width = image_A.size[0]
...
def randomRotation(image_A, image_B, label, image_A_diff, image_B_sharp):
    mode = Image.BICUBIC
    if random.random() > 0.8:
        random_angle = np.random.randint(-15, 15)
        image_A = image_A.rotate(random_angle, mode)
        image_B = image_B.rotate(random_angle, mode)
        image_A_diff = image_A_diff.rotate(random_angle, mode)
        image_B_sharp = image_B_sharp.rotate(random_angle, mode)
        label = label.rotate(random_angle, mode)
    return image_A, image_B, label, image_A_diff, image_B_sharp


def colorEnhance(image_A, image_B, image_A_diff, image_B_sharp):
    ...
    image_A_diff = ImageEnhance.Brightness(image_A_diff).enhance(bright_intensity)
    image_B_sharp = ImageEnhance.Brightness(image_B_sharp).enhance(bright_intensity)
    ...
    image_A_diff = ImageEnhance.Contrast(image_A_diff).enhance(contrast_intensity)
    image_B_sharp = ImageEnhance.Contrast(image_B_sharp).enhance(contrast_intensity)
    ...
    image_A_diff = ImageEnhance.Color(image_A_diff).enhance(color_intensity)
    image_B_sharp = ImageEnhance.Color(image_B_sharp).enhance(color_intensity)
    ...
    image_A_diff = ImageEnhance.Sharpness(image_A_diff).enhance(sharp_intensity)
    image_B_sharp = ImageEnhance.Sharpness(image_B_sharp).enhance(sharp_intensity)
    return image_A, image_B, image_A_diff, image_B_sharp
...
class SemiChangeDataset(data.Dataset):
    def __init__(self, root, trainsize, train_ratio=1,mosaic_ratio=0.75):
        ...
        self.image_root_A =  root + 'A/'
        self.image_root_B =  root + 'B/'
        self.image_root_A_diff =  root + 'A_diff/'
        self.image_root_B_sharp =  root + 'B_sharp/'
        self.gt_root = root + 'label/'
        ...
        self.images_A = [self.image_root_A + f for f in os.listdir(self.image_root_A) if f.endswith('.jpg') or f.endswith('.png') or f.endswith('.tif')]
        self.images_B = [self.image_root_B + f for f in os.listdir(self.image_root_B) if f.endswith('.jpg') or f.endswith('.png') or f.endswith('.tif')]
        self.images_A_diff = [self.image_root_A_diff + f for f in os.listdir(self.image_root_A_diff) if f.endswith('.jpg') or f.endswith('.png') or f.endswith('.tif')]
        self.images_B_sharp = [self.image_root_B_sharp + f for f in os.listdir(self.image_root_B_sharp) if f.endswith('.jpg') or f.endswith('.png') or f.endswith('.tif')]
        self.gts = [self.gt_root + f for f in os.listdir(self.gt_root) if f.endswith('.jpg')
                    or f.endswith('.png')]
        ...
        self.images_B = sorted(self.images_B)
        self.gts = sorted(self.gts)
        self.images_A_diff = sorted(self.images_A_diff)
        self.images_B_sharp = sorted(self.images_B_sharp)
        self.filter_files()
        """
    ...
    def __getitem__(self, index):
        ...
        p_ratio = random.random()
        if p_ratio > self.mosaic_ratio:
            image_A, image_B, gt, image_A_diff, image_B_sharp = self.load_img_and_mask(index)
            image_A, image_B, gt, image_A_diff, image_B_sharp = cv_random_flip(image_A, image_B, gt, image_A_diff, image_B_sharp)
            image_A, image_B, gt, image_A_diff, image_B_sharp = randomCrop(image_A, image_B, gt, image_A_diff, image_B_sharp)
            image_A, image_B, gt, image_A_diff, image_B_sharp = randomRotation(image_A, image_B, gt, image_A_diff, image_B_sharp)
            image_A,image_B, image_A_diff, image_B_sharp = colorEnhance(image_A,image_B, image_A_diff, image_B_sharp)
            gt = randomPeper(gt)
        else:
            image_A, image_B, gt, image_A_diff, image_B_sharp = self.load_mosaic_img_and_mask(index)
            image_A, image_B, gt, image_A_diff, image_B_sharp = cv_random_flip(image_A, image_B, gt, image_A_diff, image_B_sharp)
            image_A, image_B, gt, image_A_diff, image_B_sharp = randomRotation(image_A, image_B, gt, image_A_diff, image_B_sharp)
            image_A,image_B, image_A_diff, image_B_sharp = colorEnhance(image_A,image_B, image_A_diff, image_B_sharp)
            gt = randomPeper(gt)
        ...
        image_A = self.img_transform(image_A)
        image_B = self.img_transform(image_B)
        image_A_diff = self.img_transform(image_A_diff)
        image_B_sharp = self.img_transform(image_B_sharp)
        gt = self.gt_transform(gt)
        if index in self.gt_list:
            with_label=True
        else:
            with_label = False
        return image_A, image_B, gt,with_label, image_A_diff, image_B_sharp
    ...
    def load_img_and_mask(self, index):
        ...
        B = Image.open(self.images_B[index]).convert('RGB')
        mask = Image.open(self.gts[index]).convert('L')
        A_diff = Image.open(self.images_A_diff[index]).convert('RGB')
        B_sharp = Image.open(self.images_B_sharp[index]).convert('RGB')
        return A, B, mask, A_diff, B_sharp
    ...
    def load_mosaic_img_and_mask(self, index):
       ...
       # 1. 接收 5 个返回值
       img_a_a, img_a_b, mask_a, diff_a, sharp_a = self.load_img_and_mask(indexes[0])
       img_b_a, img_b_b, mask_b, diff_b, sharp_b = self.load_img_and_mask(indexes[1])
       img_c_a, img_c_b, mask_c, diff_c, sharp_c = self.load_img_and_mask(indexes[2])
       img_d_a, img_d_b, mask_d, diff_d, sharp_d = self.load_img_and_mask(indexes[3])
       ...
       # 2. 使用新添加的支持 5 个变量的裁剪函数
       croped_a_a, croped_a_b, mask_crop_a, diff_crop_a, sharp_crop_a = randomCrop_Mosaic_Semi(img_a_a.copy(), img_a_b.copy(), mask_a.copy(), diff_a.copy(), sharp_a.copy(), crop_size_a[0], crop_size_a[1])
       croped_b_a, croped_b_b, mask_crop_b, diff_crop_b, sharp_crop_b = randomCrop_Mosaic_Semi(img_b_a.copy(), img_b_b.copy(), mask_b.copy(), diff_b.copy(), sharp_b.copy(), crop_size_b[0], crop_size_b[1])
       croped_c_a, croped_c_b, mask_crop_c, diff_crop_c, sharp_crop_c = randomCrop_Mosaic_Semi(img_c_a.copy(), img_c_b.copy(), mask_c.copy(), diff_c.copy(), sharp_c.copy(), crop_size_c[0], crop_size_c[1])
       croped_d_a, croped_d_b, mask_crop_d, diff_crop_d, sharp_crop_d = randomCrop_Mosaic_Semi(img_d_a.copy(), img_d_b.copy(), mask_d.copy(), diff_d.copy(), sharp_d.copy(), crop_size_d[0], crop_size_d[1])
       ...
       diff_crop_a, sharp_crop_a = np.array(diff_crop_a), np.array(sharp_crop_a)
       diff_crop_b, sharp_crop_b = np.array(diff_crop_b), np.array(sharp_crop_b)
       diff_crop_c, sharp_crop_c = np.array(diff_crop_c), np.array(sharp_crop_c)
       diff_crop_d, sharp_crop_d = np.array(diff_crop_d), np.array(sharp_crop_d)
       ...
       top_diff = np.concatenate((diff_crop_a, diff_crop_b), axis=1)
       bottom_diff = np.concatenate((diff_crop_c, diff_crop_d), axis=1)
       img_diff = np.concatenate((top_diff, bottom_diff), axis=0)
       
       top_sharp = np.concatenate((sharp_crop_a, sharp_crop_b), axis=1)
       bottom_sharp = np.concatenate((sharp_crop_c, sharp_crop_d), axis=1)
       img_sharp = np.concatenate((top_sharp, bottom_sharp), axis=0)

       # 5. 格式整理并转回 PIL Image
       mask = np.ascontiguousarray(mask)
       img_a = np.ascontiguousarray(img_a)
       img_b = np.ascontiguousarray(img_b)
       img_diff = np.ascontiguousarray(img_diff)
       img_sharp = np.ascontiguousarray(img_sharp)
       ...
       # 返回 5 个变量，与 __getitem__ 对应
       return img_a, img_b, mask, img_diff, img_sharp
